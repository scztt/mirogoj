...
//		insertTarget = server.asTarget;
	...
	addFromArray {
		...
		array.do({
			...
			if( i==0, {
				lastGroup = this.prNewGroup( name, target, \tail );
			},{			
				lastGroup = this.prNewGroup( name, lastGroup, \after );
			});
	...
	prNewGroup {
		...
		group = CtkGroup.new( addAction: action, target: target, server: server );
		if( isPlaying, { group.play() });
		this.put( key, group );
		^group
	}
	...
	put {
		| key, val ... args |
		if( isPlaying, { this.prPlayGroup( val ) });
		super.put( key, val)
	}
